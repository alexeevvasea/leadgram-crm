...
async def suggest_response(
    ...
    messages = await db.messages.find(
        {"client_id": request.client_id, "user_id": user_id}
...
async def get_close_deal_tips(
    ...
    return AIResponse(
        response="Вот несколько советов для закрытия сделки:",
        suggestions=tips
    )
...
async def generate_custom_response(
    ...
    if "цена" in prompt or "стоимость" in prompt:
        ...
        response = "Спасибо за интерес к объявлению! Готов ответить на все ваши вопросы."
